...
const addWatcher = (done) => {
    ...
    gulp.watch(allFiles.concat(['./template/**/*', './src/**/*', '!./template/styles/css/main.css', './index.js', './preconfig.json']), gulp.series(createTemplate, buildPackageStatics, styles, bundleAndMinify)).on("change", function(file) {
        var filePath = path.join(`${__dirname}\\${file}`);
        var hash = md5File.sync(filePath);
