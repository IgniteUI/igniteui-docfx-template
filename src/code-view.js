(function ($) {
  $.widget("custom.codeView", {
    ...
    _stackblitzText: "StackBlitz",
    _csbText: "Codesandbox",
    renderFooter: function (liveEditingButtonsClickHandler, explicitEditor) {
      ...
      if (!(this._isIE || this._isEdge)) {
        ...
        if (!explicitEditor) {
          ...
          var $liveEditingButton = $("<button>", {class: this.css[explicitEditor]});
          $liveEditingButton.text(this["_" + explicitEditor + "Text"]);
          $liveEditingButton.css("font-weight", 500);

          $footerContainer.append('<span class="editing-label">Edit in: </span>').
          append($liveEditingButton).
          appendTo(this._options.$footer);
          $liveEditingButton.on("click", liveEditingButtonsClickHandler.bind($liveEditingButton, $(this.element)))
        } else {
